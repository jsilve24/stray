...
such model which is based on multivarite Gaussian processes. As Gaussian
processes are a type of lazy learner, I have named this model after a lazy dog: 
a Basset Hound. Hense this model is called *basset*. The *basset* model
can be written as 
$$
\begin{align}
...
*i.e.*, $\Gamma_{ij} = K(X_i, X_j)$. Many different possibel kernel 
...
history of comming up with rediculous names for statistical models and software.] dataset from 
...
just a different kernel function that also relates the covarance between
...
requres that `Theta` and `Gamma` be given as matrix functions rather than as
matricies. That is `Theta` must be a function that given `X` outputs a $D-1 \times N$ matrix 
...
sysmtem ($ALR_D$). This may be generalized in the future. 

Here we will just specify the Kernel parameters manually, more generally these
parameters can be choosen by cross-validation. 
