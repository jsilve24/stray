...
*stray* [@silverman2019] is a loose accronym for **"(Bayesian) Multinomial Logistic-Normal Models"**. 
...
Crohn's Disease (CD) to healthly contols [@gevers2014]. To do this I will fit a pibble model using 
CD status, inflamation status and age as covariates (plus a constatn intercept term). 
...
I strongly recomend users perform prior predictive checks to make sure their
...
coordinates with non-zero effect for diagnosisCD
...
collapsed into 1 line. Afer this the last three lines of the model can be reexpanded using fully conjugate sampling schemes
...
\end{align}
$$
where $A=(I_N + X^T \Gamma, X)$ and $T_{(D-1)\times N}$ refers to the Matrix T-distribution the $(D-1)\times N$ matrix $\eta$ with log density given by 
$$\log T_{(D-1)\times N}(\eta | \upsilon, \Theta X, \Xi, A) \propto -\frac{\upsilon+N-D-2}{2}\log | I_{D-1}+\Xi^{-1}(\eta-\Theta X)A^{-1}(\eta-\Theta X)^T |.$$
