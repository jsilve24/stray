...
dat <- RISK_CCFA %>% 
  ...
  filter_taxa(function(x) sum(x > 5) > 0.25*length(x), TRUE)
```
...
Next specify priors, in particular use phylogeneticly
informed prior
```{r}
upsilon <- ntaxa(dat)+3
m <- vcv.phylo(phy_tree(dat), corr=TRUE)
GG <- cbind(diag(ntaxa(dat)-1), -1)
#Xi <- (upsilon-ntaxa(dat)-2)*GG%*%diag(ntaxa(dat))%*%t(GG)
Xi <- (upsilon-ntaxa(dat)-2)*GG%*%m%*%t(GG)
image(Xi)
Theta <- matrix(0, ntaxa(dat)-1, nrow(X))
Gamma <- diag(nrow(X))

fit <- mongrel(Y, X, upsilon, Theta, Gamma, Xi, verbose=FALSE)

# Name taxa by family
names_categories(fit) <- tax_table(dat)[,"Family"]
