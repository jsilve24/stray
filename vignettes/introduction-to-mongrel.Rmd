...
The function `mongrel` is the main function in the *mongrel* package. It is 
...
data out of the call to mongrel or by passing `Y=NULL`^[X however must be passed
if sampling $\eta$ from the prior is desired.]  

```{r}
priors <- mongrel(NULL, X, upsilon, Theta, Gamma, Xi)  
priors
```

Except for the main function `mongrel`, most functions in the *mongrel* package
...
`mongrel` takes expects inputs and outputs in the "default" coordinate system;
...
As mentioned above however, I have designed *mongrel* to work with many 
...
Also rather than refitting the model using the `mongrel` function, we will 
...
Note unlike the main *mongrel* function, the `refit` method can be called
...
the same object as if you had called `mongrel` with just the priors. 
...
*mongrel* also has a simpler function that summarizes the poterior predictive check. 
