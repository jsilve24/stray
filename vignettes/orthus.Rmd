...
Stray can be used for jointly modeling multivariate count data and multivariate Gaussian data. For example, this would be a reasonable model to jointly model 16S microbiome data and metabolomics data jointly. Because of the "two-headed" nature of this model, e.g., two observed datasets, I named this model *orthus* (a two-headed dog and brother of Cerberus in Greek Mythology)[https://en.wikipedia.org/wiki/Orthrus]. The *orthus* model can be written as
...
and $\Theta$ undergo a similar expansion to accomidate the second dataset. 

# Joint modeling of Microbial 16S data and Metabolomics
To demonstrate *orthus* I will perform a toy analysis on data from @kashyap2013 and made available by @callahan2016 as part of their recently published microbioem data anlaysis workflow [@callahan2016]. I follow the data preprocessing of @callahan2016 we just don't drop taxa but instead amalgamate those that don't pass filtering to a category called "other". I do this to maintain the proper variance in the multinomial model.
...
I will specify that I have weak prior beleif that the taxa are independent in terms of their log absolute abundance. 
...
substantial covariance between the taxa and the metabolites and I assume the metabolites are likely indpednent. 
...
Note the stucture of this prior, everything is independent but there is a moderate positive covariance
...
For `orthusfit` objects they only transform the log-ratio components of paramters
...
akin to cannonical correlation analysis (CCA). I could look at how well the metabolites
...
(or between any two high-dimensinal multivariate datasets) is really difficult! 
...
prior beleifs with your prior. But how often can that really be done perfectly?
As such I would caution users, don't run out and try applying *orthus* on every 
possible dataset with massive dimensions. Think carefully about what taxa or metabolites
(or whatever else you are looking at) you really care about and analyze those. 
Use a bit of expert knowledge to guide your science. 

That said, please do not take the above comment as a particular limitation of *orthus*. 
It's not that some other model will overcome this issue. It's a fundamental issue with 
answering a very very difficult question (e.g., the covariance between high-dimensional datasets)
using limited data. 

Alright, thats probably enough philosophising for an R package Vignette. I hope you enjoy
*orthus*. 
